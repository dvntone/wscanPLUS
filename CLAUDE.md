...
- **Android**: create `android/local.properties` with `GOOGLE_MAPS_API_KEY=` and `CROWDSEC_CTI_API_KEY=`; place `google-services.json` at `android/app/google-services.json`
...
CTI and Gemini are currently informational tracks; they do not yet feed back into confidence scoring (tracked as runtime drift — see `PROJECT_OVERVIEW.md`).
...
- Google Maps API: Android maps and heatmap — **do not replace**
...
**Kotlin toolchain:** AGP 9.x ships with built-in Kotlin — do not add `org.jetbrains.kotlin.android`. `jvmTarget` defaults from `compileOptions.targetCompatibility`. JDK 17, compileSdk/targetSdk 36, minSdk 24.

## PR and Branch Workflow

- Branch names must start with `claude/`, `copilot/`, or `dvntone/`
- One open PR at a time across all agents; every PR references exactly one GitHub Issue
- Open all PRs as **DRAFT** (`gh pr create --draft`); mark Ready for Review only when CI is green and all checklist items are complete
