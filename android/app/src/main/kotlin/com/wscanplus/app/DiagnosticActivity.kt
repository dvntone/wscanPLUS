...
class DiagnosticActivity : Activity() {
    ...
    private fun refreshDiagnostics() {
        ...
        val cellCard = WscanUi.card(content)
        WscanUi.sectionTitle(cellCard, "Cell towers")
        val observations = binder.latestCellObservations
        if (observations.isEmpty()) {
            WscanUi.body(cellCard, "No cell observations. READ_PHONE_STATE required on API 29+.", muted = true)
        } else {
            WscanUi.metricRow(cellCard, "Towers", observations.size.toString())
            observations.take(3).forEachIndexed { i, obs ->
                val mccMnc = if (obs.mcc != null && obs.mnc != null) "${obs.mcc}/${obs.mnc}" else "—"
                WscanUi.metricRow(cellCard, "#${i + 1} type", obs.type.name)
                WscanUi.metricRow(cellCard, "#${i + 1} MCC/MNC", mccMnc, WscanUi.COLOR_MUTED)
                WscanUi.metricRow(cellCard, "#${i + 1} CI", obs.ci?.toString() ?: "—", WscanUi.COLOR_MUTED)
                WscanUi.metricRow(cellCard, "#${i + 1} signal", obs.rsrp?.let { "$it dBm" } ?: "—", cellSignalColor(obs.rsrp))
            }
            if (observations.size > 3) WscanUi.body(cellCard, "+ ${observations.size - 3} more", muted = true)
        }
    }
    ...
    private fun cellSignalColor(rsrp: Int?): Int =
        when {
            rsrp == null -> WscanUi.COLOR_MUTED
            rsrp >= -80 -> WscanUi.COLOR_OK
            rsrp >= -100 -> WscanUi.COLOR_WARN
            else -> WscanUi.COLOR_BAD
        }
